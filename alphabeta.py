...
def get_winner(board: List, n: int) -> Optional[str]:
    ...
    for c in range(n):
        ...
        if all(x == col[0] for x in col) and isinstance(col[0], str):
            return col[0]
    # chéo chính
    diag = [board[i * n + i] for i in range(n)]
    if all(x == diag[0] for x in diag) and isinstance(diag[0], str):
        return diag[0]
    # chéo phụ
    adiag = [board[i * n + (n - 1 - i)] for i in range(n)]
    if all(x == adiag[0] for x in adiag) and isinstance(adiag[0], str):
        return adiag[0]
    return None


def minimax_alpha_beta(position: List, n: int, depth: int, alpha: float, beta: float, maximizing: bool) -> int:
    ...
    if maximizing:
        ...
        for cell in get_available_cells(position):
            position[cell - 1] = X
            eval_val = minimax_alpha_beta(position, n, depth + 1, alpha, beta, False)
...
def find_best_move(position: List, n: int, ai: str) -> int:
